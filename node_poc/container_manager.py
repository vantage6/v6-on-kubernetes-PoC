        ...
        * Proposed (more accurate) description:
        ...
        for pod in job_pods.items:
            ...
            if pod.status.phase == "Succeeded":      
                self.log.info(f"Found a completed job/POD with a (k8s) Succeded status: {pod.metadata.name} /v6-job {job_id}. Returning result with v6-COMPLETED status")          
                ...
                self.log.info(f"Reading output generated by job/POD {pod.metadata.name} /v6-job {job_id} at {succeded_job_output_file}")          
                ...
                self.log.info(f"Found a completed POD with a (k8s) Failed status: {pod.metadata.name} /v6-job {job_id}. Returning result with v6-CRASHED status")
